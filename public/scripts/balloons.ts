...
function makeBall(w: number, h: number): Ball {
  ...
  if (Math.abs(vy) < 0.25) vy = vy < 0 ? -0.25 : 0.25;

  return { x, y, r, vx, vy };
}

function resizeCanvas(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D): Size {
...
function step(balls: Ball[], w: number, h: number): void {
...
function draw(ctx: CanvasRenderingContext2D, balls: Ball[], w: number, h: number): void {
...
function initBalloons(): void {
  var canvas = document.getElementById("bgBalloons") as HTMLCanvasElement | null;
  if (!canvas) return;
